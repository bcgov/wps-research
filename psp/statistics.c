...
Numerical Recipes
******************************************************************************/
void ks(float *data,float *xvalues, unsigned long nbins, float (*func)(float),float *d, float *prob)
{
	float probks(float alam);
	unsigned long j;
	float dt,en,ff;
...
void ks_testVector(float *VC, int Ncol, float min_value, float max_value, int Nbin, int pdf_case, float *d, float *prob)
{
	...
	if (pdf_case == 0) { 
			/* Gaussian Case */
			ks(ExpDataCDF,XHist,Nbin,GaussCDF,d,prob);
	}
	else if (pdf_case == 1){
			/* Exponential Case */
			...
			/* Makes the test */	
			ks(ExpDataCDF,XHist,Nbin,ExpCDF,d,prob);
	}
	else if (pdf_case == 2){
		    /* Rayleigh Case */
			...
			/* Makes the test */	
			ks(ExpDataCDF,XHist,Nbin,RayCDF,d,prob);		
    }
    else if (pdf_case == 3){
		    /* Uniform Case */
		    /* Makes the test */	
			ks(ExpDataCDF,XHist,Nbin,UnifCDF,d,prob);		
    }
}
