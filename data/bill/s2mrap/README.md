...
This library helps fill the pixels with objectives like cloud/haze/... with the previous recent pixel, but lighting will be adjusted at preference.


Summary
---

User can change the configurations and settings before running mrap. It's not mandatory to perform adjusted lighting, however doing so might help with better visualization. 
...
+ Mrapping
    ...
    **cloud_threshold**: float [0, 1] -> Recommended 0.005
    ...
    **min_cloud_prop_trigger**: float [0, 1] -> Default = 0.01

        The minimum percentage of pixels that are cloud-masked (cloud or not based on cloud threshold) to actually mrap on it (can set to 1% for example, so if cloud content is actually less than 1% it will not mrap, set to 0 means just 1 pixel of cloud will trigger the algorithm).


    **max_mrap_days**: int -> Default = 7

        Only mrap within this number of days. Prepare the dates before hand. 


    2. Adjusted lighting parameters

    **sample_prop**: float [0, 1] -> Recommended 0.1

        Random sampling from the non cloud data (of both days), and build a model to find a path to adjust the lighting condition.
