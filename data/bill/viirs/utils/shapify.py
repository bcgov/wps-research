...
Usage:
    ...
    python -m viirs.utils.shapify /data/viirs/2025/282/*.nc --bbox -126.07 52.18 -124.37 53.21 -w 8
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert VNP14IMG fire pixels to Shapefile in UTM projection.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m viirs.utils.vnp14_to_shp /data/viirs/2025
  python -m viirs.utils.vnp14_to_shp /data/viirs/2025 --utm-zone 9 --hemisphere N
  python -m viirs.utils.vnp14_to_shp VNP14IMG.A2025245.1012.002.nc
  python -m viirs.utils.vnp14_to_shp /data/viirs/2025/282/*.nc --bbox -126.07 52.18 -124.37 53.21 -w 8
        """,
    )
    ...
    parser.add_argument('--utm-zone', type=int, default=None,
                        help='UTM zone number (auto-detected if omitted)')
    parser.add_argument('--hemisphere', type=str, default=None, choices=['N', 'S'],
                        help='Hemisphere (auto-detected if omitted)')
    parser.add_argument('--bbox', type=float, nargs=4, metavar=('W', 'S', 'E', 'N'), default=None)
    ...
    if n_files == 1 or args.workers <= 1:
        results = [process_file(f, utm_zone=args.utm_zone, hemisphere=args.hemisphere,
                                bbox=args.bbox, output=args.output) for f in nc_files]
    else:
        worker_fn = partial(_worker, utm_zone=args.utm_zone, hemisphere=args.hemisphere, bbox=args.bbox)
