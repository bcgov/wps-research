...
class GIF():
    ...
    def __update(
        ...
        #Plot 2: the next date
        self.im2.set_data(
            self.__raw2plot(new_data)
        )

        self.title2.set_text(f"index: {idx} | date={date}")
    ...
    def __layout_and_run(
        ...
        self.imL = ax1.imshow(plotting_data)
        ax1.set_title(f"Reference date: {self.date_list[0]}")

        temp = np.zeros((plotting_data.shape[0], plotting_data.shape[1]))

        #The middle data
        self.im2 = ax2.imshow(temp, vmin=0, vmax=1)
        self.title2 = ax2.set_title(f"date={self.date_list[0]}")

        #method 1 on 2 imageries
        self.im3 = ax3.imshow(temp, vmin=0, vmax=1)
        self.title3 = ax3.set_title(f"dNBR")

        #method 2 on 2 imageries
        self.im4 = ax4.imshow(temp, vmin=0, vmax=1)
        self.title4 = ax4.set_title(f"Change Detection")

        #method 3 on 2 imageries
        self.im5 = ax5.imshow(temp, vmin=0, vmax=1, cmap = 'gray')
        self.title5 = ax5.set_title(f"Change Detection (SWIR wins)")

        #method 3 on 2 imageries
        self.im6 = ax6.imshow(temp, vmin=0, vmax=1, cmap = 'gray')
        self.title56= ax6.set_title(f"Change Detection (NIR wins)")
