 ...
 * 
 * Usage: ./zprdemo filename.csv field_x field_y field_z
 * 
...
void drawPoints() {
    ...
    if (renderMode == GL_SELECT) {
        // In selection mode: draw each point with its own name
        float4 *pts = NULL;
        
        // Map buffer to read positions
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        ...
        // Normal render mode: draw all points at once
        // Color picked points differently
        if (myPickNames.empty()) {
            glColor3f(0.2f, 0.8f, 1.0f);
            glDrawArrays(GL_POINTS, 0, numPoints);
        } else {
            // Draw non-picked points
            glColor3f(0.2f, 0.8f, 1.0f);
            glDrawArrays(GL_POINTS, 0, numPoints);
            
            // Highlight picked points
            glPointSize(1.2f);
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        fprintf(stderr, "Usage: %s filename.csv field_x field_y field_z\n", argv[0]);
        fprintf(stderr, "Example: %s points.csv x y z\n", argv[0]);
        return 1;
    }
    
    const char *filename = argv[1];
    const char *field1 = argv[2];
    const char *field2 = argv[3];
    const char *field3 = argv[4];
    
    printf("Loading %s with fields: %s, %s, %s\n", filename, field1, field2, field3);
    
    // Load CSV data
    std::vector<float4> points;
    if (!loadCSV(filename, field1, field2, field3, points)) {
    ...
    char title[256];
    snprintf(title, sizeof(title), "Point Cloud: %s (%d points)", filename, (int)points.size());
