 ...
 * 
 * Usage: ./mrap_qa [optional: specific L2_* folder]
 * 
 * Requires: GDAL library
 * Compile: g++ -O2 -o mrap_qa mrap_qa.cpp $(gdal-config --cflags --libs)
 */
#include <iostream>
#include <vector>
#include <string>
...
// Returns: number of pixels where NaN status is inconsistent across bands
int check_nan_consistency(GDALDataset* dataset, int& total_nan_pixels, int& total_valid_pixels) {
    ...
    for (int b = 0; b < nbands; ++b) {
        bands[b].resize(xsize * ysize);
        GDALRasterBand* band = dataset->GetRasterBand(b + 1);
        band->RasterIO(GF_Read, 0, 0, xsize, ysize, 
...
// Create a NaN mask from a dataset (true = has valid data in all bands)
std::vector<bool> create_valid_mask(GDALDataset* dataset) {
    int xsize = dataset->GetRasterXSize();
    int ysize = dataset->GetRasterYSize();
    int nbands = dataset->GetRasterCount();
...
QAResult run_qa_on_folder(const std::string& folder) {
    ...
    for (size_t idx = 0; idx < mrap_files.size(); ++idx) {
        ...
        GDALDataset* dataset = (GDALDataset*)GDALOpen(mf.path.c_str(), GA_ReadOnly);
        if (!dataset) {
            result.passed = false;
