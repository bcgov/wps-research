/* 20260205: from raster_negate.cpp */
#include"misc.h"

int main(int argc, char ** argv){
  if(argc < 2){
    err("raster_log [hyperspec cube] # take the log of values\n");
  }
  ...
  str ofn(str(argv[1]) + str("_log.bin")); // output file name
  ...
  for0(k, nband){
    ...
    for0(i, nrow){
      ...
      for0(j, ncol){
        ij = ix + j;
        out[ij] = (float)log((double)dat[ij]);
      }
    }
  }
