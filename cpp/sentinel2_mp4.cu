 ...
 * - S2C_MSIL2A_20251014T192401_N0511_R099_T09UYU_... (timestamp from 3rd underscore field)
 * - yyyymmdd_mrap.bin (timestamp from first 8 digits)
 * 
 * Compile with:
...
std::string extractTimestamp(const std::string& filename) {
    // Supported patterns:
    // Pattern 1: S2C_MSIL2A_20251014T192401_N0511_R099_T09UYU_...
    //            Fields separated by underscore, timestamp is 3rd field (index 2)
    // Pattern 2: yyyymmdd_mrap.bin (e.g., 20251014_mrap.bin)
    //            Timestamp is the first 8 characters (yyyymmdd)
    
    // Check for Pattern 2: yyyymmdd_mrap.bin
    // Must start with 8 digits followed by _mrap.bin (case insensitive)
    if (filename.length() >= 13) {  // "yyyymmdd_mrap.bin" minimum length
        bool starts_with_8_digits = true;
        for (int i = 0; i < 8 && starts_with_8_digits; i++) {
            if (!isdigit(filename[i])) {
                starts_with_8_digits = false;
            }
        }
        
        if (starts_with_8_digits && filename[8] == '_') {
            // Check for _mrap.bin suffix (case insensitive)
            std::string lower_name = filename;
            std::transform(lower_name.begin(), lower_name.end(), lower_name.begin(), ::tolower);
            if (lower_name.find("_mrap.bin") != std::string::npos) {
                return filename.substr(0, 8);  // Return yyyymmdd
            }
        }
    }
    
    // Pattern 1: S2C_MSIL2A_TIMESTAMP_...
    // Fields separated by underscore, timestamp is 3rd field (index 2)
...
int main(int argc, char** argv) {
    ...
    // Allocate GPU buffers for histogram computation (will resize if needed)
    float* d_hist_buffer = nullptr;
    size_t d_hist_buffer_size = 0;
    ...
    while (hist_images_processed < (int)g_image_files.size()) {
        ...
        if (img_data.valid && img_data.host_data) {
            ...
            size_t this_image_pixels = (size_t)img_data.header.samples * img_data.header.lines;
            size_t this_image_size = this_image_pixels * bands * sizeof(float);
            
            // Reallocate GPU buffer if this image is larger
            if (this_image_size > d_hist_buffer_size) {
                if (d_hist_buffer) {
                    CUDA_CHECK(cudaFree(d_hist_buffer));
                }
                CUDA_CHECK(cudaMalloc(&d_hist_buffer, this_image_size));
                d_hist_buffer_size = this_image_size;
            }
            
            // Upload to GPU using this image's actual size
            CUDA_CHECK(cudaMemcpyAsync(d_hist_buffer, img_data.host_data, this_image_size,
    ...
    // CUDA kernel configuration (block sizes - grids computed per-image)
    dim3 block_1d(256);
    dim3 block_2d(16, 16);
    ...
    while (next_process_index < (int)g_image_files.size()) {
        ...
        // Use this image's actual dimensions
        int this_width = img_data.width;
        int this_height = img_data.height;
        size_t this_pixels = (size_t)this_width * this_height;
        size_t this_float_size = this_pixels * g_image_bands * sizeof(float);
        size_t this_rgb_size = this_pixels * 3;
        
        // Kernel configurations for this image's dimensions
        dim3 this_grid_1d((this_pixels + block_1d.x - 1) / block_1d.x);
        
        // Upload float data to GPU
        float* d_float_temp;
        CUDA_CHECK(cudaMalloc(&d_float_temp, this_float_size));
        CUDA_CHECK(cudaMemcpyAsync(d_float_temp, img_data.host_data,
                                    this_float_size, cudaMemcpyHostToDevice, stream));
        
        // Allocate temp RGB buffer
        uint8_t* d_rgb_temp;
        CUDA_CHECK(cudaMalloc(&d_rgb_temp, this_rgb_size));
        
        // Scale and convert to RGB8
        scaleAndConvertKernel<<<this_grid_1d, block_1d, 0, stream>>>(
            d_float_temp,
            d_rgb_temp,
            this_width, this_height, g_image_bands,
            g_global_bounds[0].min_val, g_global_bounds[0].max_val,
            g_global_bounds[1].min_val, g_global_bounds[1].max_val,
            g_global_bounds[2].min_val, g_global_bounds[2].max_val
        );
