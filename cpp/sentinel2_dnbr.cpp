...
/* find index of the band with wavelength freq_nm in file file_name*/
...
int main(int argc, char ** argv){
  if(argc < 3){
    err("sentinel2_dnbr [s2 preimg] [s2 post] [opt'l flag: writeNBR] # NIR, SWR rq'd");
  }
  size_t nrow, ncol, nband, nrow2, ncol2, nband2, np, i, j, k, n, ij;
  float *out, *dat1, *dat2, *b1, *b2, *b3, *out2, *out3;
  long int bi[3];

  str fn_pre(argv[1]), fn_pst(argv[2]); /* binaries to read */
  str hfn1(hdr_fn(fn_pre)), hfn2(hdr_fn(fn_pst));

  str ofn(fn_pst + "_dnbr.bin"), ofn2(fn_pre + "_nbr.bin"), ofn3(fn_pst + "_nbr.bin");
  str ohn(hdr_fn(ofn, true)), ohn2, ohn3;
  
  vector<str> s1, s2; // band-name lists, two dates
  str s_NIR("842nm"), s_SWR("2190nm"); // required wavelengths
  ...
  if(argc > 3){
    ohn2 = hdr_fn(fn_pre + "_nbr.hdr", true);
    ohn3 = hdr_fn(fn_pst + "_nbr.hdr", true);
  ...
  hwrite(ohn, nrow, ncol, 1, 4);
  bwrite(out, ofn, nrow, ncol, 1);

  if(argc > 3){
    hwrite(ohn2, nrow, ncol, 1, 4);
    bwrite(out2, ofn, nrow, ncol, 1);
    hwrite(ohn3, nrow, ncol, 1, 4);
    bwrite(out3, ofn, nrow, ncol, 1); // still should write band names/ mapinfo, 3 headers
  }
