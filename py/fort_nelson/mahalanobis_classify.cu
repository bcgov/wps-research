...
./classify_mahalanobis training.bin input.bin input.hdr [output.bin]
    ...
    int pad, long long start_pixel, long long n_pixels)
{
    ...
    // Find nearest exemplar using Mahalanobis distance
    ...
    output[pixel_idx] = exemplar_labels[best_idx];
}
...
int main(int argc, char** argv) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s training.bin input.{tif|bin} [input.hdr] [output.bin]\n", argv[0]);
        fprintf(stderr, "  For ENVI: %s training.bin input.bin input.hdr [output.bin]\n", argv[0]);
        fprintf(stderr, "  For TIFF: %s training.bin input.tif [output.bin]\n", argv[0]);
        return 1;
    }
    ...
    if (output_file == NULL) {
        ...
        if (last_dot != NULL) {
            ...
            snprintf(auto_output, sizeof(auto_output), "%.*s_classification.bin", base_len, input_file);
        } else {
            snprintf(auto_output, sizeof(auto_output), "%s_classification.bin", input_file);
        }
    ...
    for (int e = 0; e < n_exemplars; e++) {
        fread(&all_labels[e], sizeof(unsigned char), 1, f);
        fread(&all_means[e * patch_dim], sizeof(float), patch_dim, f);
        
        // Skip covariance, read inverse covariance
        float* cov = (float*)malloc(patch_dim * patch_dim * sizeof(float));
        fread(cov, sizeof(float), patch_dim * patch_dim, f);
        free(cov);
        
        fread(&all_inv_covs[e * patch_dim * patch_dim], sizeof(float), patch_dim * patch_dim, f);
    }
    fclose(f);
