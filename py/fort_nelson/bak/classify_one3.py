'''20260109 classifier seems to be working. modes:

    classify_one.py [annotations shapefile] # this calculates patch statistics ( all *.tif images ) and saves to global_stats.pkl 

    classify_one.py [annotations shapefile] [input tif file] # this recalls global stats from global_stats.pkl and creates and ENVI format output file for the given tif file. For testing the process on one image..

    classify_one.py # no arguments: this loads global_stats.pkl and generates ENVI format classification maps for all *.tif files in present directory
'''
#!/usr/bin/env python3
import sys, os
import warnings
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        mean_covs = global_stats['mean_covs']
    ...
    srcs = list(training.items())
    for i, (src, d) in enumerate(srcs, 1):
        ...
        for lbl in (0, 1):
            if out[lbl][0] is not None:
                S[lbl].append(out[lbl])

        print_progress_bar(
            i,
            len(srcs),
            prefix=f"Training ({src}):",
            suffix="Done"
        )
    ...
    for lbl in (0, 1):
        if S[lbl]:
            means = [m for m, _ in S[lbl]]
            covs  = [c for _, c in S[lbl]]
            mean_covs[lbl] = (np.mean(means, axis=0), np.mean(covs, axis=0))
        else:
