...
#!/usr/bin/env python3
"""
stage.py - Move directories from /data/ram/ to /ram/ (ramdisk) with parallel rsync
Usage: python stage.py [directory] [--threads N]
...
def main():
    parser = argparse.ArgumentParser(description='Stage data from /data/ram/ to /ram/')
    parser.add_argument('directory', nargs='?', help='Directory to stage (default: current directory)')
    ...
    if source_size > available_space:
        print(f"\nError: Insufficient space on /ram/", file=sys.stderr)
    ...
    print(f"\n✓ Successfully staged {base_name} to /ram/")
