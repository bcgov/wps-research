...
class ENVIImageAnnotator:
    def __init__(self, root):
        ...
        self.toggle_button = tk.Button(control_frame, text="Show Classification", command=self.toggle_view, state=tk.DISABLED)
        ...
        self.tk_image = None
        self.classification_tkimg = None
    ...
    def load_image(self, path):
        ...
        self.classification_tkimg = None
        self.show_classification = False
        self.toggle_button.config(state=tk.DISABLED, text="Show Classification")
    ...
    def on_mouse_up(self, event):
        if self.current_rect:
            ...
            if abs(x1 - x0) < 5 or abs(y1 - y0) < 5:
                ...
                self.rectangles.append((rect, label))
            self.current_rect = None
    ...
    def classify(self):
        ...
        if len(self.rectangles) == 0:
            messagebox.showerror("Error", "No labeled rectangles to train model.")
            return

        self.status_var.set("Training model...")
        self.root.update()

        # Compute mean and covariance for each class based on labeled rectangles
        mean_covs = compute_patch_mean_cov(self.full_image_data, [r[0] for r in self.rectangles], [r[1] for r in self.rectangles])

        self.status_var.set("Classifying image...")
        self.root.update()

        total_rows = self.full_image_data.shape[0]
        progress = [0]  # Use list to mutate in closure

        def progress_callback():
            progress[0] += 1
            # Update terminal progress bar
            print_progress_bar(progress[0], total_rows, prefix='Classification Progress:', suffix='Complete', length=50)
            # Update GUI status every 10 rows to reduce flicker
            if progress[0] % 10 == 0 or progress[0] == total_rows:
                self.status_var.set(f"Classifying image... {progress[0]}/{total_rows} rows")
                self.root.update()

        classified = classify_by_gaussian_parallel(self.full_image_data, mean_covs, patch_size=7, progress_callback=progress_callback)

        # Save result
        outpath = os.path.splitext(self.dataset.GetDescription())[0] + "_classified.bin"
        save_classification_envi(classified, self.dataset, outpath)

        # Prepare classification overlay image (green/red mask)
        overlay = np.zeros_like(self.full_image_data[:, :, :3], dtype=np.uint8)
        overlay[classified == 1] = [0, 255, 0]   # green
        overlay[classified == 0] = [255, 0, 0]   # red
        overlay_img = Image.fromarray(overlay)
        self.classification_tkimg = ImageTk.PhotoImage(overlay_img)

        self.show_classification = False
        self.toggle_button.config(state=tk.NORMAL, text="Show Classification")
        self.status_var.set("Classification complete. Use 'Show Classification' to toggle view.")

    def toggle_view(self):
        if self.classification_tkimg is None:
            return
        self.show_classification = not self.show_classification
        self.canvas.delete("all")
        if self.show_classification:
            self.canvas.create_image(0, 0, anchor=tk.NW, image=self.classification_tkimg)
            self.toggle_button.config(text="Show Original")
            self.status_var.set("Displaying classification overlay")
        else:
            self.canvas.create_image(0, 0, anchor=tk.NW, image=self.tk_image)
            # Draw rectangles on top
            for rect, label in self.rectangles:
                color = "green" if label == 1 else "red"
                self.canvas.create_rectangle(*rect, outline=color, width=2)
            self.toggle_button.config(text="Show Classification")
            self.status_var.set("Displaying original image with labeled rectangles")
