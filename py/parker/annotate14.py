...
def classify_image(image, mean_covs, patch_size=7):
    ...
    print("Starting classification (parallel)...")
    ...
    print(f"Classification done in {time.time() - start:.1f}s")
    return classified
...
class ENVIImageAnnotator:
    def __init__(self, root):
        self.root = root
        self.root.title("ENVI Image Annotator + Classifier")
    ...
    def on_mouse_up(self, event):
        if self.current_rect:
            ...
            self.current_rect = None
            self.classify_and_save_model()

    def classify_and_save_model(self):
        ...
        mean_covs = compute_patch_mean_cov(self.image_data, coords, labels)

        # Save model
        model_path = os.path.splitext(self.filename)[0] + "_model.pkl"
    ...
    def load_model(self):
        ...
        print("Classifying using loaded model...")
